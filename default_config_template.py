"""
The exact source text written out to gazette_watcher/config.py the first
time the app runs and doesn't find that file next to it — e.g. a fresh
copy of the .exe dropped into an empty folder. Without this, the exe
would require the file to already exist (it must, since config.py is
meant to be genuinely user-editable data, not something baked into the
frozen bundle — see main.pyw's frozen-config-loading comment) and would
just crash instead of working out of the box.

Keep this in sync with gazette_watcher/config.py's own content/comments
whenever that file's structure changes — this is the "factory defaults"
version, matching what DEFAULTS in main.pyw restores via Reset to defaults.
"""
