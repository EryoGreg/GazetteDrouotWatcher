...
**`config.py` es el único archivo a editar** para todo: qué páginas vigilar, con qué frecuencia comprobar, qué tan profundo analizar, límites de notificación, tiempos de espera de alertas, etc. — cada ajuste tiene un comentario explicativo (o edítelos desde la pestaña Configuración del panel de control, vea abajo). Se encuentra en `%LOCALAPPDATA%\GazetteDrouotWatcher\config.py` (creado allí automáticamente en la primera ejecución — no junto al `.exe`), de modo que sobrevive a mover o reemplazar el propio `.exe`. Tras cualquier cambio ahí, tendrá efecto en la siguiente ejecución, **excepto** `POLL_INTERVAL_MINUTES`, que también requiere volver a hacer clic en **Instalar** en el panel de control para actualizar la tarea real del Programador de tareas de Windows con el nuevo intervalo.

## Panel de control (GUI)

**Esta es la aplicación** — un único `.exe` autónomo, sin necesidad de instalar Python por separado ni de archivos de script en la máquina que lo ejecute. Una ventana de escritorio para todo: instalar / activar / desactivar / desinstalar la tarea programada (mediante la API nativa del Programador de tareas directamente, sin PowerShell), y un panel de configuración (con "Restablecer valores predeterminados" si algo se estropea) en lugar de editar el archivo de configuración a mano. El icono de la bandera cambia el idioma de la interfaz (inglés, 中文, español, हिन्दी, العربية, português, русский, français, 日本語, alemán — sigue por defecto el idioma de Windows, y si no es compatible usa inglés); el icono sol/luna alterna claro/oscuro (sigue por defecto el tema de Windows). Ambas elecciones se guardan en `%LOCALAPPDATA%\GazetteDrouotWatcher\gui_prefs.json`.

**Si solo tiene el `.exe`:** haga doble clic en `GazetteDrouotWatcher.exe` — no hay nada más que instalar, ni nada más que necesite junto a él. Es totalmente autónomo: `config.py`, `gui_prefs.json`, `state/` y `logs/` se crean todos bajo `%LOCALAPPDATA%\GazetteDrouotWatcher\` en la primera ejecución, no junto al propio `.exe`, de modo que nunca esparce archivos en la carpeta desde la que se ejecute. Haga clic en **Instalar** en la ventana para registrar la tarea programada — a partir de ahí, la comprobación se hace automáticamente en segundo plano, con el intervalo definido en `config.py`, sin que esta ventana (ni la aplicación) necesite permanecer abierta, y se inicia sola de nuevo tras cada reinicio del PC.
...
`%LOCALAPPDATA%\GazetteDrouotWatcher\logs\watcher.log` tiene el detalle completo de cada ejecución — revíselo primero si las notificaciones dejan de aparecer.
