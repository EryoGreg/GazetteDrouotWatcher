...
TRANSLATIONS = {
    "en": {
        ...
        "btn_open_activity_log": "Activity log",
        ...
        "welcome_body": (
            ...
            "who wanted to stay up to date on art auctions and art news without checking manually. The full "
            ...
            "You might see a note asking you to restart as Administrator. Just looking at your "
            "settings never needs that — you can check everything freely. But anything that actually "
            "changes what's scheduled to run automatically in the background — installing, enabling, "
            "disabling, or uninstalling the periodic check, or saving a settings change — does need "
            "it, because that's what Windows requires before letting a program schedule itself. Until "
            "you restart as Administrator, those buttons simply stay greyed out rather than pretending "
            "to work; nothing is broken, you just need that one extra step first.\n\n"
            ...
            "either; the app simply stops checking until you turn it back on. Two things worth "
            "knowing: **if you move this app to a different folder, just open it once from its new "
            "location before the next scheduled check, so it knows where to find itself.** And if you "
            "delete it (for example, dragging it to the Recycle Bin) without clicking Uninstall first, "
            "the next scheduled check notices it's gone and cleans everything up by itself — so it "
            "never keeps running in the background with nothing left to control it.\n\n"
    ...
    "fr": {
        ...
        "btn_open_activity_log": "Journal d'activité",
        ...
        "welcome_body": (
            ...
            "ami qui voulait rester informé des ventes aux enchères d'art et de l'actualité du marché de "
            "l'art sans avoir à vérifier lui-même. Le code source complet est public — https://github.com/EryoGreg/GazetteDrouotWatcher "
            ...
            "Consulter vos réglages ne l'exige jamais — vous pouvez tout vérifier librement. Mais tout ce "
            "qui modifie réellement ce qui est programmé pour s'exécuter automatiquement en arrière-plan — "
            "« Installer », « Activer », « Désactiver » ou « Désinstaller » la vérification périodique, ou "
            "« Enregistrer » une modification des réglages — en a besoin, car c'est ce que Windows exige "
            "avant qu'un programme puisse se programmer lui-même. Tant que vous n'avez pas redémarré en "
            "tant qu'administrateur, ces boutons restent simplement grisés au lieu de faire semblant de "
            "fonctionner ; rien n'est cassé, il vous manque juste cette étape supplémentaire.\n\n"
            ...
            "réactiviez. Deux choses à savoir : **si vous déplacez cette application vers un autre dossier, "
            "ouvrez-la simplement une fois depuis son nouvel emplacement avant la prochaine vérification "
            "programmée, afin qu'elle sache où se trouver.** Et si vous la supprimez (par exemple en la "
            "faisant glisser vers la Corbeille) sans avoir cliqué sur « Désinstaller » au préalable, la "
            "prochaine vérification programmée constatera son absence et nettoiera tout automatiquement — "
            "elle ne continuera donc jamais à tourner en arrière-plan sans que personne ne puisse plus la "
            "contrôler.\n\n"
    ...
    "es": {
        ...
        "btn_open_activity_log": "Registro de actividad",
        ...
        "welcome_body": (
            ...
            "para un amigo que quería mantenerse al día con las subastas de arte y las noticias del mundo "
            "del arte sin tener que revisarlo manualmente. El código fuente completo es público — "
            ...
            "Puede que vea un aviso pidiendo 'Reiniciar como administrador'. Consultar su configuración "
            "nunca lo necesita — puede revisarlo todo con total libertad. Pero cualquier cosa que realmente "
            "cambie lo que está programado para ejecutarse automáticamente en segundo plano — 'Instalar', "
            "'Activar', 'Desactivar' o 'Desinstalar' la revisión periódica, o 'Guardar' un cambio de "
            "configuración — sí lo necesita, porque eso es lo que exige Windows antes de dejar que un "
            "programa se programe a sí mismo. Hasta que reinicie como administrador, esos botones "
            "simplemente permanecen en gris en lugar de fingir que funcionan; no hay nada roto, solo falta "
            "ese paso adicional.\n\n"
            ...
            "hasta que la vuelva a activar. Dos cosas que conviene saber: **si mueve esta aplicación a otra "
            "carpeta, ábrala una vez desde su nueva ubicación antes de la próxima revisión programada, para "
            "que sepa dónde encontrarse.** Y si la elimina (por ejemplo, arrastrándola a la Papelera de "
            "reciclaje) sin hacer clic antes en 'Desinstalar', la próxima revisión programada notará que ya "
            "no está y lo limpiará todo por sí sola — así que nunca seguirá ejecutándose en segundo plano "
            "sin que nadie pueda controlarla.\n\n"
    ...
    "de": {
        ...
        "btn_open_activity_log": "Aktivitätsprotokoll",
        ...
        "welcome_body": (
            ...
            "einen Freund, der über Kunstauktionen und Kunstnachrichten auf dem Laufenden bleiben wollte, "
            "ohne selbst nachzusehen. Der vollständige Quellcode ist öffentlich einsehbar — "
            ...
            "auszuwählen. Nur Ihre Einstellungen anzusehen erfordert das nie — Sie können alles frei "
            "ansehen. Aber alles, was wirklich ändert, was automatisch im Hintergrund ausgeführt werden "
            "soll — 'Installieren', 'Aktivieren', 'Deaktivieren' oder 'Deinstallieren' der regelmäßigen "
            "Prüfung, oder 'Speichern' einer Einstellungsänderung — braucht das, weil Windows das verlangt, "
            "bevor ein Programm sich selbst einplanen darf. Bis Sie als Administrator neu starten, bleiben "
            "diese Buttons einfach ausgegraut, statt so zu tun, als würden sie funktionieren; es ist nichts "
            "kaputt, es fehlt nur dieser eine zusätzliche Schritt.\n\n"
            ...
            "einfach auf zu prüfen, bis Sie sie wieder einschalten. Zwei Dinge sind gut zu wissen: **Wenn Sie "
            "diese App in einen anderen Ordner verschieben, öffnen Sie sie einmal von ihrem neuen Standort "
            "aus, bevor die nächste geplante Prüfung stattfindet, damit sie weiß, wo sie sich befindet.** Und "
            "wenn Sie sie löschen (zum Beispiel, indem Sie sie in den Papierkorb ziehen), ohne vorher auf "
            "'Deinstallieren' zu klicken, bemerkt die nächste geplante Prüfung, dass sie fehlt, und räumt "
            "von selbst alles auf — so läuft nie etwas dauerhaft im Hintergrund weiter, ohne dass Sie es "
    ...
    "pt": {
        ...
        "btn_open_activity_log": "Registo de atividade",
        ...
        "welcome_body": (
            ...
            "para um amigo que queria manter-se atualizado sobre leilões de arte e notícias do mundo da "
            "arte sem ter de verificar manualmente. O código-fonte completo é público — https://github.com/EryoGreg/GazetteDrouotWatcher "
            ...
            "Pode aparecer um aviso a pedir para 'Reiniciar como administrador'. Consultar as suas "
            "definições nunca precisa disso — pode verificar tudo livremente. Mas qualquer coisa que "
            "realmente altere o que está agendado para correr automaticamente em segundo plano — "
            "'Instalar', 'Ativar', 'Desativar' ou 'Desinstalar' a verificação periódica, ou 'Guardar' uma "
            "alteração das definições — precisa disso, porque é o que o Windows exige antes de deixar um "
            "programa agendar-se a si próprio. Até reiniciar como administrador, esses botões ficam "
            "simplesmente cinzentos em vez de fingir que funcionam; nada está avariado, só falta esse passo "
            "extra.\n\n"
            ...
            "ativar. Duas coisas a saber: **se mudar esta aplicação para outra pasta, abra-a uma vez a partir "
            "do novo local antes da próxima verificação agendada, para que saiba onde se encontra.** E se a "
            "eliminar (por exemplo, arrastando-a para a Reciclagem) sem clicar primeiro em 'Desinstalar', a "
            "próxima verificação agendada nota que já não está lá e limpa tudo sozinha — para que nunca "
            "continue a correr em segundo plano sem que ninguém a possa controlar.\n\n"
    ...
    "ru": {
        ...
        "btn_open_activity_log": "Журнал активности",
        ...
        "welcome_body": (
            ...
            "который хотел быть в курсе арт-аукционов и новостей мира искусства, не проверяя сайт вручную. Полный "
            ...
            "пригодиться.\n\n"
            "Вы можете увидеть сообщение с просьбой «Перезапустить от имени администратора». Простой "
            "просмотр настроек никогда этого не требует — вы можете свободно всё проверять. Но всё, что "
            "действительно меняет то, что запланировано выполняться автоматически в фоновом режиме — "
            "«Установить», «Включить», «Отключить» или «Удалить» периодическую проверку, или «Сохранить» "
            "изменение настроек — этого требует, потому что именно это нужно Windows, прежде чем разрешить "
            "программе самой себя запланировать. Пока вы не перезапустите систему от имени администратора, "
            "эти кнопки просто остаются серыми, а не делают вид, что работают; ничего не сломано, просто "
            "нужен этот один дополнительный шаг.\n\n"
            ...
            "вы не включите его снова. Ещё две вещи, которые полезно знать: **если вы перенесёте приложение "
            "в другую папку, откройте его один раз из нового места перед следующей запланированной "
            "проверкой, чтобы оно знало, где находится.** А если вы удалите его (например, перетащив в "
            "Корзину), не нажав сначала «Удалить», следующая запланированная проверка обнаружит его "
            "отсутствие и сама всё уберёт за собой — так что оно никогда не будет вечно работать в фоне "
    ...
    "zh": {
        ...
        "welcome_body": (
            ...
            "工作。\n\n"
            "这个小工具是 Grégoire Pessiot 在业余时间做的，是送给一位朋友的礼物，因为她想随时了解"
            "艺术品拍卖和艺术新闻动态，又不想每次都手动去查。完整源代码是公开的——"
            "https://github.com/EryoGreg/GazetteDrouotWatcher——如果你好奇（或者只是想确认里面没有什么"
            "见不得人的东西），欢迎查看每一行代码。也欢迎把它分享给任何可能用得上的人。\n\n"
            "你可能会看到一条提示，要求“以管理员身份重新启动”。只是查看设置从来都不需要这样——你可"
            "以随意查看一切。但任何真正会改变后台自动运行安排的操作——对定时检查执行“安装”“启用”"
            "“禁用”或“卸载”，或者“保存”设置更改——都需要这样做，因为这是 Windows 在允许程序自行"
            "安排运行之前所要求的。在你以管理员身份重新启动之前，这些按钮只会保持灰色，而不会假装能正"
            "常工作；没有任何东西损坏，只是还差这一步。\n\n"
            ...
            "是停止检查，直到你重新打开它。还有两件事值得知道：**如果你把这个程序移动到别的文件夹，请"
            "在下一次定时检查之前从新位置打开它一次，让它知道自己现在在哪里。**如果你没有先点击“卸载”"
            "就直接删除它（比如把它拖进回收站），下一次定时检查会发现它已经不在了，并自动把一切清理"
            "干净——这样它就不会在后台一直运行下去而没人能够控制它。\n\n"
    ...
    "ja": {
        ...
        "welcome_body": (
            ...
            "この小さなツールは、Grégoire Pessiot が空き時間に作ったもので、美術品オークションやアー"
            "ト関連ニュースを自分で確認せずに把握していたいという友人へのプレゼントとして生まれました。ソースコードはす"
            ...
            "「管理者として再起動」を求めるメッセージが表示されることがあります。設定を確認するだけな"
            "らこれは一切必要ありません——自由にすべて確認できます。ただし、バックグラウンドで自動的"
            "に実行される内容を実際に変更する操作——定期チェックの「インストール」「有効化」「無効"
            "化」「アンインストール」、または設定変更の「保存」——には必要です。これは、プログラムが"
            "自分自身を自動実行するようスケジュールする前に Windows が求めることだからです。管理者と"
            "して再起動するまでは、これらのボタンは動作するふりをするのではなく、単にグレーアウトした"
            "ままになります。壊れているわけではなく、その一手間が必要なだけです。\n\n"
            ...
            "リは再び有効にするまでチェックを止めるだけです。知っておくとよいことが二つあります。**もし"
            "このアプリを別のフォルダーに移動した場合は、次の定期チェックが行われる前に、その新しい場"
            "所から一度アプリを開いてください。そうすることで、アプリは自分の居場所を把握できます。**ま"
            "た、「アンインストール」をクリックせずに（たとえばごみ箱にドラッグして）アプリを削除して"
            "しまった場合でも、次の定期チェックの際にアプリがなくなっていることが検知され、自動的にす"
            "べてが片付けられます——そのため、誰にもコントロールできないままバックグラウンドで動き続け"
            "ることはありません。\n\n"
    ...
    "hi": {
        ...
        "btn_open_activity_log": "गतिविधि लॉग",
        ...
        "welcome_body": (
            ...
            "था, जो बिना खुद बार-बार जाँचे कला नीलामियों (art auctions) और कला जगत की खबरों से अपडेट रहना "
            "चाहता था। इसका पूरा सोर्स कोड सार्वजनिक है — https://github.com/EryoGreg/GazetteDrouotWatcher — तो "
            ...
            "है।\n\n"
            "आपको 'व्यवस्थापक के रूप में पुनः आरंभ करें' जैसा कोई संदेश दिख सकता है। सिर्फ अपनी सेटिंग्स "
            "देखने के लिए इसकी कभी ज़रूरत नहीं होती — आप बेझिझक सब कुछ देख सकते हैं। लेकिन जो भी चीज़ "
            "पृष्ठभूमि में अपने-आप चलने वाले शेड्यूल को वाकई बदलती है — नियमित जाँच को 'इंस्टॉल करें', "
            "'सक्षम करें', 'अक्षम करें' या 'अनइंस्टॉल करें' करना, या सेटिंग्स में बदलाव को 'सहेजें' करना — "
            "उसके लिए यह ज़रूरी है, क्योंकि किसी प्रोग्राम को खुद को शेड्यूल करने की अनुमति देने से पहले "
            "Windows को यही चाहिए होता है। जब तक आप व्यवस्थापक के रूप में पुनः आरंभ नहीं करते, तब तक ये "
            "बटन काम करने का दिखावा करने के बजाय बस धुंधले (निष्क्रिय) बने रहते हैं; कुछ भी टूटा नहीं है, "
            "बस यह एक अतिरिक्त कदम बाकी है।\n\n"
            ...
            "नहीं होता; ऐप बस तब तक जाँच करना बंद कर देता है जब तक आप इसे दोबारा चालू न करें। दो बातें जान "
            "लेना अच्छा रहेगा: **अगर आप इस ऐप को किसी दूसरे फ़ोल्डर में ले जाते हैं, तो अगली शेड्यूल की गई "
            "जाँच से पहले इसे एक बार नए स्थान से खोल लें, ताकि इसे पता चल सके कि यह अब कहाँ है।** और अगर आप "
            "इसे बिना पहले 'अनइंस्टॉल करें' पर क्लिक किए हटा देते हैं (जैसे कि इसे रीसाइकल बिन में खींच "
            "कर), तो अगली शेड्यूल की गई जाँच इसके गायब होने को पहचान लेती है और खुद-ब-खुद सब कुछ साफ़ कर "
            "देती है — ताकि यह कभी भी पृष्ठभूमि में हमेशा के लिए चलता न रहे, जिस पर किसी का कोई नियंत्रण न "
            "हो।\n\n"
    ...
    "ar": {
        ...
        "btn_open_activity_log": "سجل النشاط",
        ...
        "welcome_body": (
            ...
            "صُنعت هذه الأداة الصغيرة على يد Grégoire Pessiot في وقت فراغه، كهدية لصديق أراد متابعة "
            "مزادات الأعمال الفنية وأخبار عالم الفن دون الحاجة إلى التحقق يدويًا. الشيفرة المصدرية كاملة ومتاحة للعموم "
            ...
            "قد تظهر لك رسالة تطلب 'إعادة التشغيل كمسؤول'. مجرد الاطلاع على إعداداتك لا يتطلب ذلك أبدًا — "
            "يمكنك التحقق من كل شيء بحرية. لكن أي شيء يغيّر فعليًا ما هو مجدوَل للعمل تلقائيًا في الخلفية "
            "— 'تثبيت' أو 'تفعيل' أو 'تعطيل' أو 'إلغاء التثبيت' للفحص الدوري، أو 'حفظ' تغيير في الإعدادات "
            "— يحتاج إلى ذلك، لأن هذا ما يتطلبه Windows قبل السماح لبرنامج ما بجدولة نفسه. وإلى أن تُعيد "
            "التشغيل كمسؤول، تبقى هذه الأزرار رماديةً ببساطة بدلاً من أن تتظاهر بأنها تعمل؛ لا شيء معطّل، "
            "أنت فقط بحاجة إلى هذه الخطوة الإضافية أولاً.\n\n"
            ...
            "تفعيله. هناك أمران يستحقان المعرفة: **إذا نقلت هذا التطبيق إلى مجلد مختلف، فافتحه مرة واحدة من "
            "موقعه الجديد قبل الفحص المجدوَل التالي، حتى يعرف أين يجد نفسه.** وإذا حذفته (على سبيل المثال "
            "بسحبه إلى سلة المحذوفات) دون النقر على 'إلغاء التثبيت' أولاً، فإن الفحص المجدوَل التالي سيلاحظ "
            "اختفاءه وينظّف كل شيء بنفسه — بحيث لا يبقى يعمل في الخلفية إلى الأبد دون أن يكون لأحد سيطرة "
            "عليه.\n\n"
